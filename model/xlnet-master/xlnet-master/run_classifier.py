...
class DoctorProcessor(DataProcessor):
	def get_labels(self):
		return [0, 1]

	def get_train_examples(self, data_dir):
		return self._create_examples(data_dir + "/processed_filled_by_doctors.csv")
	...
	def _create_examples(self, file):
		...
		dictv = {"CARDIOLOGY": 0, "FAMILY PRACTICE/PRIMARY CARE": 1}

		with open(file) as infile:
			reader = csv.DictReader(infile)

			for row in reader:
				examples.append(InputExample(guid="unused_id", text_a = row['CC'], text_b = None, label = dictv[row['Specialty']]))
				
		return examples

# processed_filled_by_doctors
